...
public class KafkaSourceAvro {
  protected static final Logger LOGGER = LoggerFactory.getLogger(KafkaSourceAvro.class);
  ...
  protected KafkaConsumer<String, GenericRecord> buildKafkaConsumer(final JsonNode config) {
    ...
    public AutoCloseableIterator<AirbyteMessage> consumeMessages(final JsonNode config){
      final KafkaConsumer<String, GenericRecord> consumer = buildKafkaConsumer(config);
      ...
      int pollCount = 0;
      while (true) {
        final ConsumerRecords<String, GenericRecord> consumerRecords = consumer.poll(Duration.of(100, ChronoUnit.MILLIS));
            ...
            try {
              ...
              return new AirbyteMessage()
                  ...
                  .withRecord(new AirbyteRecordMessage()
                      ...
                      .withData(JsonLoader.fromString(record.value().toString())));
            } catch (final Exception e) {
              LOGGER.error("Exception attempting to read message from topic: ", e);
            }
          }
          return endOfData();
        }
